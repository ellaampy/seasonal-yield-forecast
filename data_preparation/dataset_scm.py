...
class YieldDataset_SCM(Dataset):
    def __init__(self, predictor_path, yield_path, norm=None, years=None, 
                 ...
                 scm_bin=8, bias_adjusted=True, scm_features=["tavg","tmax", "tmin", "prec"],
                 scm_truncation=None, return_type='zero_filled'):
        """
        ...
        Parameters:
            ...
            - scm_features(list): list of features to use from the scm data
        ...
        print(self.scm_df.columns, self.scm_df.shape)
        ...
        #print(self.df[[c for c in self.df.columns if c.startswith('tavg')]].head(2))
        ...
        #print(self.df[[f'{col}_new' for col in replace_cols]])
    ...
    #     # ====================== FEATURE SELECTION START ==============================
        ...
        #get index for static features
        self.static_idx = [feature_selector.index(elem) for elem in static_features if elem in feature_selector]
    ...
    def __getitem__(self, idx):
        ...
        # calculate how many SCM timesteps remain after truncation
        scm_range = self.scm_timesteps - (self.max_timesteps-self.temporal_truncation[1])
        ...
        # returns observed data filled with scm and zeros after truncate
            ...
            merged_data = self.truncated_data[idx]
            if scm_range > 0:
                for i in range(self.truncated_data.shape[2]):
                    if (i not in self.scm_idx) & (i not in self.static_idx):
                        merged_data[-scm_range:, i] = 0
